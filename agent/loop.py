...
OnStatus = Callable[[str, str], Awaitable[None]]       # 状态回调：(action, message)，action: start/end
...
class AgentLoop:
    ...
    async def _notify_status(self, action: str, message: str) -> None:
        """通知 UI 层状态变化（开始/结束 spinner）

        Args:
            action: "start" 开始显示 spinner / "end" 结束 spinner
            message: 状态描述，如 "思考中..." / "执行 read_file..."
        """
        if self.on_status:
            try:
                await self.on_status(action, message)
            except Exception:
                pass  # UI 回调失败不影响主流程
        ...
        try:
            while iteration < self.cfg.agent.max_iterations:
                ...
                if self.memory.is_near_limit():
                    ...
                    await self._notify_status("start", "压缩上下文中...")
                    compress_result = await self.memory.llm_compress(self.llm)
                    await self._notify_status("end", "压缩上下文中...")
                    if compress_result:
                        self._debug.log(
                            "agent",
                ...
                finish_reason = ""
                _thinking_shown = False  # 是否已显示"思考中"

                # 通知 UI 开始思考（spinner 显示）
                await self._notify_status("start", "思考中...")
                ...
                async for chunk in self.llm.chat(messages, tools=tool_schemas):
                    ...
                    if chunk["type"] == "text_delta":
                        # 第一个文本块到达时结束"思考中" spinner，开始流式输出
                        if not _thinking_shown:
                            await self._notify_status("end", "思考中...")
                ...
                # LLM 调用结束：如果一直没输出文本（如直接调用工具），结束 spinner
                if not _thinking_shown:
                    await self._notify_status("end", "思考中...")
    ...
    async def _execute_tool_internal(self, tool_call: dict, func_name: str, func_args: dict):
        ...
        # 执行工具（包装为可独立取消的 Task）
        result = None
        _tool_status_shown = False
        try:
            # 检查取消信号
            if abort_event and abort_event.is_set():
                result = ToolResult(success=False, output="", error="工具被取消")
            else:
                # 通知 UI 开始执行工具（spinner 显示）
                await self._notify_status("start", f"执行 {func_name}...")
                _tool_status_shown = True
            ...
            # 结束工具执行 spinner
            if _tool_status_shown:
                await self._notify_status("end", f"执行 {func_name}...")
            # 清理注册
            if tool_call_id:
